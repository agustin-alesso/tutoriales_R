...
```{r}
# Agregar capa histograma
p <- p + geom_histogram()
p
```

3. Si es necesario ajustar la escala de colores con `scales_`. En este caso vamos a probar un gradiente de colores con `scale_fill_brewer()`.

```{r}
# Modificar escala de colores
p <- p + scale_fill_brewer(palette = "RdYlGn", type = "div")
p
```
...
```{r}
p <- p + facet_wrap(~ as.factor(Fecha))
p
```
...
p <- ggplot(terneros, aes(x = Peso, fill = Procedencia)) +
  geom_histogram() +
  scale_fill_brewer(palette = "RdYlGn", type = "div") +
  facet_wrap(~ as.factor(Fecha)) +
  coord_flip() + 
...
p + coord_cartesian() +
  geom_vline(data = med, aes(xintercept = media, color = Procedencia)) +
